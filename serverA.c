...
#define UDP_PORT "30936" //the port main server will be connected to
...
int main(void) {
    ...
    FILE *data;
    data = fopen("data1.txt","r"); //reading the txt file
    char datafile[255];
    ...
    while(!feof(data)){
      ...
      if (number == 0 && datafile[0] != '0'){
        ...
        countryName[i][0] = '0'; //insert a 0 in front of country name to indicate it's from server A
        i++;
      }
    }
    ...
    fscanf(data, "%[^\n]%*c", datafile); //scan the first line of data1.txt
    while(!feof(data)){
      fscanf(data, "%[^\n]%*c", datafile);
   ...
   fclose(data);
    ...
    printf("The server A is up and running using UDP on port "UDP_PORT"\n\n");
    ...
    while(1){
        ...
        char updatedRequest[] = "0";

        printf("The server A has received request for finding possible friends of User %d in %s\n\n", userRequest, countryRequest);

        strcat(updatedRequest,countryRequest); //insert a 0 in front of country name to indicate it's from server A
        ...
        if (ufound == 0){
            ...
            printf("The server A is searching for possible friends for User %d...\n\n", userRequest);
            ...
            if(connected == user_num_per_country[countryIndex] - 1){ //if the requested user is already connected to everyone, send signal -1
                ...
                printf("The server A has sent the result to Main Server\n\n");
            }
